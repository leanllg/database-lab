...
select u.* from USERS u, FOLLOW f
  where f.UID = u.UID 
  and f.UIDFLED = (select UID from USERS u2 where u2.NAME = '张三')
  order by BYEAR DESC, UID ASC;

-- q2:查找没有被任何人点赞的博文ID、标题以及发表者姓名，
-- 并将结果按照标题字符顺序排列
select u.NAME, b.BID, b.TITLE from USERS u, MBLOG b
  where b.UID = u.UID 
...
select b.BID from MBLOG b, TOPDAY t
  where b.UID in (select UID from USERS where BYEAR >= 2000 and CITY = '武汉')
  and t.BID = b.BID;
...
select UID, BYEAR, CITY from USERS where BYEAR > 2010
union
select UID, BYEAR, CITY from USERS where BYEAR < 1970;

-- q6:统计每个城市的用户数
select CITY, count(UID) as USERS_NUM from USERS group by CITY;

-- q7:统计每个城市的每个出生年份的用户数，并将结果按照城市的升序排列，
-- 同一个城市按照出生用户数的降序排列其相应的年份
select CITY, BYEAR, count(UID) as NUM from USERS 
  group by CITY, BYEAR
...
select BID from THUMB t, USERS u
...
create view TOP_10 as
  ...
  from (
    select MBLOG.BID, TITLE, USERS.UID, NAME from MBLOG
    inner join TOPDAY t
    on t.BID = MBLOG.BID
    and t.TYEAR = datename(year, getdate())
    and t.TMONTH = datename(month, getdate())
    and t.TDAY = datename(day, getdate())
    and t.TNO <= 10
    inner join USERS
